...
export default function SlideShow({ slides, autoPlayInterval = 6000, height = '75vh' }: Props) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: isTablet ? '55vh' : height, minHeight: isMobile ? '300px' : isTablet ? '380px' : '500px', overflow: 'hidden', background: '#000', borderRadius: '0 0 24px 24px' }}>
      {/* Background with Ken Burns — all slides rendered, opacity controlled */}
      {slides.map((s, i) => {
        const kbItem = kenBurns[i % kenBurns.length];
        return (
          <motion.div
            key={`bg-${i}`}
            animate={{
              opacity: i === current ? 1 : 0,
              ...(i === current ? kbItem.animate : kbItem.initial),
            }}
            transition={{
      ...
      {/* Content */}
      <div style={{ position: 'relative', zIndex: 2, height: '100%', display: 'flex', alignItems: 'center', paddingBottom: isMobile ? '50px' : isTablet ? '50px' : '60px' }}>
        <div style={{ padding: isMobile ? '0 1.25rem' : isTablet ? '0 2rem' : '0 clamp(0rem, 27vw, 32rem)', maxWidth: '100%', width: '100%', display: 'flex', alignItems: 'center', gap: 0 }}>
          {/* Left text area */}
          <div style={{ flex: isMobile ? '1 1 100%' : '0 1 55%', minWidth: 0 }}>
            ...
            <motion.div key={`c-${current}`}>
              ...
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.4, delay: 0.15 }}>
                <span style={{
                  ...
                  padding: isMobile ? '5px 12px' : '7px 18px', border: '1px solid rgba(0,123,255,0.5)', borderRadius: '50px',
                  color: '#a4d8ff', fontSize: isMobile ? '10px' : '12px', fontWeight: 600, fontFamily: 'var(--font-english)',
                  letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: isMobile ? '0.75rem' : '1.25rem',
              ...
              <motion.h2
                ...
                style={{ fontSize: isMobile ? '1.25rem' : 'clamp(1.8rem, 4.5vw, 3.2rem)', fontWeight: 800, lineHeight: 1.2, letterSpacing: '-1px', color: '#fff', marginBottom: isMobile ? '0.5rem' : '1rem', wordBreak: 'keep-all' }}
              >
                {slide.title.split('\n').map((line, i) => (
                  <span key={i}>{line}{i < slide.title.split('\n').length - 1 && <br style={isMobile ? { display: "none" } : undefined} />}</span>
                ))}
              </motion.h2>
              ...
              <motion.p
                ...
                style={{ fontSize: isMobile ? '0.75rem' : 'clamp(0.9rem, 1.3vw, 1.1rem)', color: 'rgba(255,255,255,0.75)', lineHeight: 1.6, wordBreak: 'keep-all', maxWidth: '560px' }}
              >
                {slide.subtitle}
              </motion.p>
              ...
              {slide.highlights && (
                <motion.div
                  ...
                  style={{ display: 'flex', flexWrap: 'wrap', gap: isMobile ? '4px' : '8px', marginTop: isMobile ? '0.75rem' : '1.25rem' }}
                >
                  {slide.highlights.map((h, i) => (
                    <span key={i} style={{
                      padding: isMobile ? '3px 8px' : '5px 12px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)',
                      borderRadius: '6px', color: 'rgba(255,255,255,0.7)', fontSize: isMobile ? '10px' : '12px', fontWeight: 500,
                    }}>
                      {h}
                    </span>
                  ))}
              ...
              <motion.div
                ...
                style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '6px' : isTablet ? '6px' : '16px', marginTop: isMobile ? '0.75rem' : isTablet ? '1rem' : '1.75rem', flexWrap: 'wrap' }}
              >
                {slide.link && (
                  <a href={slide.link} style={{
                    display: 'inline-flex', alignItems: 'center', gap: '6px',
                    padding: isMobile ? '8px 14px' : isTablet ? '8px 14px' : '12px 28px',
                    background: '#007BFF', borderRadius: '50px', color: '#fff',
                    fontSize: isMobile ? '11px' : isTablet ? '11px' : '14px', fontWeight: 600,
                ...
                >
                  {isMobile || isTablet ? 'SafeD 바로가기' : 'SafeD 솔루션 바로가기'}
                ...
                <a href="/inquiries" style={{
                  ...
                  padding: isMobile ? '8px 14px' : isTablet ? '8px 14px' : '12px 24px',
                  border: '1px solid rgba(255,255,255,0.25)', borderRadius: '50px', color: '#fff',
                  fontSize: isMobile ? '11px' : isTablet ? '11px' : '14px', fontWeight: 500,
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 3 }}>
        {/* Segmented progress */}
        <div style={{ display: 'flex', gap: '3px', padding: isMobile ? '0 1.25rem' : isTablet ? '0 2rem' : '0 clamp(2rem, 8vw, 10rem)', marginBottom: '8px' }}>
          {slides.map((_, i) => (
        ...
        <div style={{
          ...
          padding: isMobile ? '8px 1.25rem' : isTablet ? '10px 2rem' : '12px clamp(2rem, 8vw, 10rem)', background: 'transparent',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '8px' : '12px' }}>
            <button onClick={prev} aria-label="이전" style={{
              width: isMobile ? '28px' : '36px', height: isMobile ? '28px' : '36px', borderRadius: '50%', border: '1px solid rgba(255,255,255,0.15)',
              background: 'transparent', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>
              <svg width={isMobile ? 10 : 14} height={isMobile ? 10 : 14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 19l-7-7 7-7" /></svg>
            </button>
            <span style={{ color: '#fff', fontSize: isMobile ? '11px' : '13px', fontFamily: 'var(--font-english)', fontWeight: 600, letterSpacing: '2px' }}>
            ...
            <button onClick={next} aria-label="다음" style={{
              ...
              <svg width={isMobile ? 10 : 14} height={isMobile ? 10 : 14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7" /></svg>
            </button>
          </div>

          {/* Slide tab buttons - hidden on mobile */}
          {!isMobile && !isTablet && (
